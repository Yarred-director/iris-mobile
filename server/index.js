...
app.post('/chat', async (req, res) => {
  try {
    ...
    // 10. Self-awareness — Iris inner world
    const selfAwarenessBlock = formatSelfAwarenessBlock(internalState);
    if (selfAwarenessBlock) promptParts.push(selfAwarenessBlock);

    // 11. Personality evolution — who she has become with this user
    ...
    Promise.all([
      ...
      // Self-awareness — Iris reflects on the conversation
      runSelfAwareness({
        supabase: req.supabase,
        userId,
        userText: message,
      ...
      // Personality evolution — gradual character growth
      evolvePersonality({
        supabase: req.supabase,
        userId,
        userText: message,
