...
async function run() {
  ...
  for (const r of reminders) {
    ...
    if (send.ok) {
      ...
      await supabase.from('reminders')
        .update({ status: 'failed', updated_at: new Date().toISOString(), meta: { ...(r.meta || {}), send_error: send.error } })
        .eq('id', r.id);
      console.log('[REMINDER] failed', r.id, send.error);
    }
  }
}
