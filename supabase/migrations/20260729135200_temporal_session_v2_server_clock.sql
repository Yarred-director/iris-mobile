-- Temporal Session v2 hardening
-- Authenticated clients must not be able to submit an arbitrary timestamp.

drop function if exists public.begin_or_touch_temporal_session(uuid, text, integer, timestamptz);
...
begin
  ...
  if not found then
    ...
    v_is_new_session :=
      ...
      or extract(epoch from (v_now - v_profile.last_interaction_at)) > p_session_timeout_seconds;

    if v_is_new_session and v_profile.last_interaction_at is not null then
      v_gap_seconds := greatest(
        0,
        floor(extract(epoch from (v_now - v_profile.last_interaction_at)))::bigint
      );
    end if;

    update public.iris_profiles
       set user_timezone = coalesce(v_timezone, v_profile.user_timezone, 'UTC'),
           relationship_started_at = coalesce(v_profile.relationship_started_at, v_now),
           current_session_started_at = case
             when v_is_new_session then v_now
             else coalesce(v_profile.current_session_started_at, v_now)
           end,
...
revoke all on function public.begin_or_touch_temporal_session(uuid, text, integer)
  from public, anon;

grant execute on function public.begin_or_touch_temporal_session(uuid, text, integer)
